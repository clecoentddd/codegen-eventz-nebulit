...
export type FactureARecupererTodoReadModel = ProcessorTodoItem & {
        entrepriseId: string;
};

// Stateless query function - reads fresh from event store each time
export async function getFactureARecupererTodoProjection(eventStore: EventStore): Promise<FactureARecupererTodoReadModel[]> {
...
const evolveState = (event: any, stateMap: Map<string, Record<string, any>>) => {
    switch (event.type) {
            ...
            case MarquerFactureRecupereeAttemptedEventName: {
                const correlationId = event.metadata?.correlation_id || event.streamId;
...
const buildTodos = (stateMap: Map<string, Record<string, any>>): FactureARecupererTodoReadModel[] => {
    const todos: FactureARecupererTodoReadModel[] = [];

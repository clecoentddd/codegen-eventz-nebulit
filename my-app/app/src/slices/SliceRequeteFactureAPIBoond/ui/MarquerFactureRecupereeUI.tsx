...
export const MarquerFactureRecupereeUI = () => {
  const [formData, setFormData] = useState<{ factureId: string; projetId: string; entrepriseId: string }>({ factureId: '', projetId: '', entrepriseId: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/marquer-facture-recuperee', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        setIsError(false);
        setFormData({ factureId: '', projetId: '', entrepriseId: '' });
      } else {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm backdrop-blur">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h3 className="text-lg font-semibold text-slate-900">Marquer Facture Récupérée</h3>

...
import { getAdminConnectionTodoProjection } from './projectionAdminConnectionTodo';

import { EntrepriseRetrouveeCommand } from './EntrepriseRetrouveeCommand';
import { createEntrepriseRetrouveeCommandHandler } from './EntrepriseRetrouveeCommandHandler';
...
const config: TodoProcessorConfig = {
    queueName: 'AdminConnectionTodo-todo-queue',
    maxRetries: 3,
    batchSize: 10
};

export class AdminConnectionTodoProcessor {
    ...
    async start(): Promise<void> {
        console.log(`Starting AdminConnectionTodo Todo Processor...`);

        // Subscribe to signals for event-driven processing
        signalMock.subscribe('AdminConnecte', async (event: any) => {
    ...
    private async processPendingTodos(): Promise<void> {
        ...
        try {
            ...
            const pendingTodos = await getAdminConnectionTodoProjection(eventStore);
    ...
    private async processTodo(todo: any): Promise<void> {
        ...
        const command: EntrepriseRetrouveeCommand = {
            streamId: ONE_STREAM_ONLY,
            type: 'EntrepriseRetrouvee',
            data: {
                entrepriseId: todo.entrepriseId,
                entrepriseNom: todo.entrepriseNom,
        ...
        const eventStore = getEventStore();
        const handler = createEntrepriseRetrouveeCommandHandler(eventStore);
        await handler(command);
        
        console.log(`[Processor] Successfully dispatched EntrepriseRetrouvee command for todo ${todo.id}`);
    ...
    async stop(): Promise<void> {
        console.log(`Stopping AdminConnectionTodo Todo Processor...`);
        this.processing = false;
    }
}

// Factory function
export function createAdminConnectionTodoProcessor(): AdminConnectionTodoProcessor {
    return new AdminConnectionTodoProcessor();
}

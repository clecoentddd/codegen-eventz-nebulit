...
export const CreerCompteUI = () => {
  const [formData, setFormData] = useState<{ clientId: string; clientEmail: string; entrepriseId: string }>({ clientId: '', clientEmail: '', entrepriseId: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setIsError(false);
        setFormData({ clientId: '', clientEmail: '', entrepriseId: '' });
      } else {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm backdrop-blur">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h3 className="text-lg font-semibold text-slate-900">Créer Compte</h3>

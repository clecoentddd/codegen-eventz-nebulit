...
export type InvitationAEnvoyerTodoReadModel = ProcessorTodoItem & {
        clientId: string;
    clientEmail: string;
    entrepriseId: string;
};
...
export async function getInvitationAEnvoyerTodoProjection(eventStore: EventStore): Promise<InvitationAEnvoyerTodoReadModel[]> {
...
const evolveState = (event: any, stateMap: Map<string, Record<string, any>>) => {
    switch (event.type) {
            case InvitationPrepareEventName: {
            ...
            case MarquerEmailEnvoyeAttemptedEventName: {
                const correlationId = event.metadata?.correlation_id || event.streamId;
...
const buildTodos = (stateMap: Map<string, Record<string, any>>): InvitationAEnvoyerTodoReadModel[] => {
    const todos: InvitationAEnvoyerTodoReadModel[] = [];

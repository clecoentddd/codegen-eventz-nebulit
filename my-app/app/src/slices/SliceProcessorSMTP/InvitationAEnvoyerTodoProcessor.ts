...
import { getInvitationAEnvoyerTodoProjection } from './projectionInvitationAEnvoyerTodo';

import { MarquerEmailEnvoyeCommand } from './MarquerEmailEnvoyeCommand';
import { createMarquerEmailEnvoyeCommandHandler } from './MarquerEmailEnvoyeCommandHandler';
...
const config: TodoProcessorConfig = {
    queueName: 'InvitationAEnvoyerTodo-todo-queue',
    maxRetries: 3,
    batchSize: 10
};

export class InvitationAEnvoyerTodoProcessor {
    ...
    async start(): Promise<void> {
        console.log(`Starting InvitationAEnvoyerTodo Todo Processor...`);

        // Subscribe to signals for event-driven processing
        signalMock.subscribe('InvitationPrepare', async (event: any) => {
    ...
    private async processPendingTodos(): Promise<void> {
        ...
        try {
            ...
            const pendingTodos = await getInvitationAEnvoyerTodoProjection(eventStore);
    ...
    private async processTodo(todo: any): Promise<void> {
        ...
        const command: MarquerEmailEnvoyeCommand = {
            streamId: ONE_STREAM_ONLY,
            type: 'MarquerEmailEnvoye',
            data: {
                clientId: randomUUID(),
        ...
        const eventStore = getEventStore();
        const handler = createMarquerEmailEnvoyeCommandHandler(eventStore);
        await handler(command);
        
        console.log(`[Processor] Successfully dispatched MarquerEmailEnvoye command for todo ${todo.id}`);
    ...
    async stop(): Promise<void> {
        console.log(`Stopping InvitationAEnvoyerTodo Todo Processor...`);
        this.processing = false;
    }
}

// Factory function
export function createInvitationAEnvoyerTodoProcessor(): InvitationAEnvoyerTodoProcessor {
    return new InvitationAEnvoyerTodoProcessor();
}

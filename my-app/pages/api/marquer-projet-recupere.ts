...
import { createMarquerProjetRecupereCommandHandler } from '../../app/src/slices/SliceProjetRequeteAPIBoond/MarquerProjetRecupereCommandHandler';
import { MarquerProjetRecupereCommand } from '../../app/src/slices/SliceProjetRequeteAPIBoond/MarquerProjetRecupereCommand';
...
export default async function handler(
  ...
  try {
    ...
    const commandHandler = createMarquerProjetRecupereCommandHandler(getEventStore());
    await commandHandler(command);

    // ---- async acknowledgement ----
    res.status(202).json({
      message: 'MarquerProjetRecupere attempt recorded',
